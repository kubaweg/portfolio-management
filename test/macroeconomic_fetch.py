...
def get_cpi():
    ...
    df = pd.read_csv(f'gus/{filename}', on_bad_lines='skip')
    ...
    df = df.rename(columns={'Data': 'Miesiac', 'Zamkniecie': 'InflacjaGUS'})

    return df[['Miesiac', 'InflacjaGUS']]
...
def get_ref():
    try:
        ...
        df_wide = df_wide.rename(columns={'Data': 'ObowiazujeOd', 'ref': 'StopaReferencyjna'})
